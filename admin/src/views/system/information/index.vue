<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="informationList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="帮助文档ID" align="center" prop="helpInformationId" />
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.helpInformationId)
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加常见问题的解答";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const helpInformationId = row.helpInformationId || this.ids
      getInformation(helpInformationId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改常见问题的解答";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.helpInformationId != null) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const helpInformationIds = row.helpInformationId || this.ids;
      this.$confirm('是否确认删除常见问题的解答编号为"' + helpInformationIds + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delInformation(helpInformationIds);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有常见问题的解答数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
